...
from ecommerce_tool.util.shipping_price import (
    get_full_order_and_shipping_details,
    get_orders_by_customer_and_date,
)
...
from omnisight.operations.helium_utils import (
    get_date_range,
...
from rest_framework.parsers import JSONParser
from bson import ObjectId
from .helium_dashboard import (
    sanitize_data,
    batch_get_sales_data_optimized,
    clean_json_floats,
)
...
def getProductList(request):
    ...
    # CACHE MARKETPLACES
    # -----------------------------
    marketplace_list = cache.get("marketplace_list")
    ...
    if not marketplace_list:
        ...
        cache.set("marketplace_list", marketplace_list, 3600)

    marketplace_map = {str(m["_id"]): m for m in marketplace_list}

    # -----------------------------
    # BUILD QUERY USING Q (FIX FOR $OR ERROR)
    # -----------------------------
    query = Q()

    if marketplace_id:
        query &= Q(marketplace_ids=ObjectId(marketplace_id))

    if category_name:
        query &= Q(category__in=category_name)

    if brand_id_list:
        brand_id_list = [ObjectId(b) for b in brand_id_list]
        query &= Q(brand_id__in=brand_id_list)
    ...
    pipeline.extend([
        {
            "$project": {
                "_id": 0,   # IMPORTANT FIX (remove ObjectId completely)
                "id": {"$toString": "$_id"},
                "product_title": 1,
                "product_id": 1,
                "sku": 1,
    ...
    # FETCH PRODUCTS
    # -----------------------------
    product_list = list(Product.objects.filter(query).aggregate(*pipeline))

    # -----------------------------
    # ENRICHMENT (O(n))
    # -----------------------------
    for product in product_list:
        product["marketplace_ins"] = []
        product["marketplace_image_url"] = []

        for mid in product.get("marketplace_ids", []):
            m = marketplace_map.get(str(mid))
            if m:
                product["marketplace_ins"].append(m["name"])
                product["marketplace_image_url"].append(m["image_url"])
...
def getBrandList(request):
    ...
    pipeline = []
    if marketplace_id != None and marketplace_id != "":
        match = {"$match": {"marketplace_id": ObjectId(marketplace_id)}}
    ...
    pipeline = [
        ...
        },
        {
            "$project": {
                ...
                "vendor_funding": {
                    "$round": [{"$ifNull": ["$vendor_funding", 0.0]}, 2]
                },
                "vendor_discount": {
                    "$round": [{"$ifNull": ["$vendor_discount", 0.0]}, 2]
                },
                "product_cost": {"$round": [{"$ifNull": ["$product_cost", 0]}, 2]},
                "referral_fee": {"$round": [{"$ifNull": ["$referral_fee", 0]}, 2]},
                "a_shipping_cost": {
                    "$round": [{"$ifNull": ["$a_shipping_cost", 0]}, 2]
                },
...
def getOrdersBasedOnProduct(request):
    ...
    matching_order_items = list(
        OrderItems.objects.filter(ProductDetails__product_id=ObjectId(product_id)).only(
            "id"
        )
    )
    ...
    orders_queryset = (
        Order.objects.filter(order_items__in=matching_order_items)
    ...
    custom_orders = list(
        custom_order.objects.filter(
            ordered_products__product_id=ObjectId(product_id)
        ).only(
            "order_id", "purchase_order_date", "order_status", "total_price", "currency"
        )
    )
        ...
        orders.append(
            {
                ...
                "order_date": c.purchase_order_date.strftime("%Y-%m-%dT%H:%M:%S.%fZ")[
                    :-3
                ],
    ...
    data = {"total_count": total_count, "orders": orders}
    return data
    ...
    if market_place_id == "custom":
        ...
        if search_query and search_query.strip():
            ...
            base_pipeline.append(
                {"$match": {"order_id": {"$regex": safe_search, "$options": "i"}}}
            )
        ...
        if search_query and search_query.strip():
            ...
            pipeline.append(
                {
                    "$match": {
                        "purchase_order_id": {"$regex": safe_search, "$options": "i"}
                    }
                }
            )
        ...
        orders = [
            o
            for o in orders
            if o.get("order_date") and o["order_date"] <= current_time_pacific
        ]
        data["orders"] = orders
        data["total_count"] = total_count
        data["status"] = ""
    marketplace_pipeline = [
        {"$project": {"_id": 0, "id": {"$toString": "$_id"}, "name": 1, "image_url": 1}}
    ]
        ...
        {"$unwind": "$marketplace_ins"},
        {
            "$project": {
                ...
                "is_global_express_enabled": {
                    "$ifNull": ["$is_global_express_enabled", False]
                },
    ...
    if order_details:
        ...
        if not merchant_shipment_cost:
            if fulfillment_channel == "SellerFulfilled":
                ...
                order_number = data.get("merchant_order_id") or data.get(
                    "customer_order_id"
                )
            if order_number:
                ...
                if full_order_details and full_order_details.get("shipments"):
                    shipment_cost = float(
                        full_order_details["shipments"][-1].get("shipmentCost", 0) or 0
                    )
                    order_obj = Order.objects(id=ObjectId(order_id)).first()
                    if order_obj:
                        order_obj.update(set__merchant_shipment_cost=shipment_cost)
        ...
        if order_items_ids:
            order_items = DatabaseModel.list_documents(
                OrderItems.objects, {"id__in": order_items_ids}
            )
            serialized_items = []
            for item in order_items:
                serialized_item = {
                    ...
                    "ProductDetails": {
                        "product_id": (
                            str(item.ProductDetails.product_id.id)
                            if item.ProductDetails and item.ProductDetails.product_id
                            else None
                        ),
                        "Title": (
                            item.ProductDetails.Title if item.ProductDetails else None
                        ),
                        "SKU": item.ProductDetails.SKU if item.ProductDetails else None,
                        "Condition": (
                            item.ProductDetails.Condition
                        ...
                        "QuantityShipped": (
                            item.ProductDetails.QuantityShipped
                            if item.ProductDetails
                            else 0
                        ),
                        "unit_price": (
                            float(item.ProductDetails.product_id.price)
                            if item.ProductDetails
                            and item.ProductDetails.product_id
                            and item.ProductDetails.product_id.price
                            else 0.0
                        ),
                    },
                    "Pricing": (
                        item.Pricing.to_mongo()
                        if hasattr(item.Pricing, "to_mongo")
                        else (dict(item.Pricing) if item.Pricing else {})
                    ),
                    "Fulfillment": {
                        "FulfillmentOption": (
                            item.Fulfillment.FulfillmentOption
                            if item.Fulfillment
                            else None
                        ),
                        "ShipMethod": (
                            item.Fulfillment.ShipMethod if item.Fulfillment else None
                        ),
                        "Carrier": (
                            item.Fulfillment.Carrier if item.Fulfillment else None
                        ),
                        "TrackingNumber": (
                        ...
                        "TrackingURL": (
                            item.Fulfillment.TrackingURL if item.Fulfillment else None
                        ),
                        "ShipDateTime": (
                            item.Fulfillment.ShipDateTime.isoformat()
                            if item.Fulfillment and item.Fulfillment.ShipDateTime
                            else None
                        ),
                    },
                    "OrderStatus": {
                        ...
                        "StatusDate": (
                            item.OrderStatus.StatusDate.isoformat()
                            if item.OrderStatus and item.OrderStatus.StatusDate
                            else None
                        ),
                    },
                    "TaxCollection": (
                        item.TaxCollection.to_mongo()
                        if hasattr(item.TaxCollection, "to_mongo")
                        else (dict(item.TaxCollection) if item.TaxCollection else {})
                    ),
                    "IsGift": item.IsGift,
                }
...
def createManualOrder(request):
    ...
    custom_product_obj["customer_order_id"] = (
        datetime.now().strftime("%Y%m%d") + str(uuid.uuid4().int)[:5]
    )
    try:
...
def listManualOrders(request):
    ...
    if sort_by != None and sort_by != "":
        sort = {"$sort": {sort_by: int(sort_by_value)}}
...
def updateManualOrder(request):
    ...
    DatabaseModel.update_documents(
        custom_order.objects, {"id": order_id}, custom_product_obj
    )
    ...
    pipeline = [
        ...
        },
        {
            "$project": {
                ...
                "discount_amount": {
                    "$ifNull": [{"$round": ["$discount_amount", 2]}, 0.0]
                },
                ...
                "ordered_products": {
                    "$map": {
                        ...
                        "in": {
                            ...
                            "quantity_price": {
                                "$ifNull": ["$$product.quantity_price", 0.0]
                            },
                        },
                    }
                },
...
#             if brand_object_ids:
#                 products = list(
#                     Product.objects.filter(brand_id__in=brand_object_ids).only("id")
#                 )
#                 product_ids_from_brands = [p.id for p in products]
#                 if product_ids_from_brands:
#                     if product_ids:
#                         product_ids = [
#                             pid for pid in product_ids if pid in product_ids_from_brands
#                         ]
#                         if not product_ids:
...
#                     {
#                         "$match": {
#                             "order_items_details.ProductDetails.product_id": {
#                                 "$in": product_ids
#                             }
#                         }
#                     },
...
#             percentage = (
#                 round((count / total_order_count) * 100, 2) if total_order_count else 0
#             )
#             results[mp.name] = {
#                 "count": count,
...
#         count = res[0].get("count", 0) if res else 0
#         order_value = round(res[0].get("order_value", 0), 2) if res else 0
#         name = DatabaseModel.get_document(
#             Marketplace.objects, {"id": marketplace_id}, ["name"]
#         ).name
#         data[name] = {
#             "value": count,
#             "percentage": "100.0%",
...
def ordersCountForDashboard(request):
    ...
    if product_ids:
        where.append("product_id IN {product_ids:Array(String)}")
        params["product_ids"] = product_ids

    where_sql = " AND ".join(where)

    # -----------------------------
    # BRAND FILTER (via Product table mapping)
    # -----------------------------
    if brand_ids:
        from omnisight.models import Product

        brand_product_ids = list(
            Product.objects.filter(brand_id__in=brand_ids)
            .values_list("id", flat=True)
        )

        if brand_product_ids:
            where_sql += " AND product_id IN {brand_products:Array(String)}"
            params["brand_products"] = [str(x) for x in brand_product_ids]
    ...
    total_query = f"""
        SELECT
            uniq(order_id) AS value,
            sum(gross_revenue) AS order_value
        FROM fact_order_items
        WHERE {where_sql}
    """

    total_count, total_value = client.query(total_query, parameters=params).result_rows[0]

    data["total_order_count"] = {
        "value": total_count or 0,
        "percentage": "100.0%",
        "order_value": round(total_value or 0, 2),
    }

    # =========================================================
    # 2. MARKETPLACE BREAKDOWN (USING channel)
    # =========================================================
    if marketplace_id == "all":
        ...
        rows = client.query(mp_query, parameters=params).result_rows

        for channel, count, value in rows:
            percentage = round((count / total_count) * 100, 2) if total_count else 0

            data[channel] = {
                "count": count or 0,
                "percentage": f"{percentage}%",
                "order_value": round(value or 0, 2),
            }

    # =========================================================
    # 3. SINGLE MARKETPLACE (channel filter)
    # =========================================================
    elif marketplace_id != "custom":

        where_mp = where_sql + " AND channel = {channel:String}"
        params_mp = dict(params)
        params_mp["channel"] = marketplace_id  # now expects Temu/Target/etc
        ...
        count, value = client.query(mp_query, parameters=params_mp).result_rows[0]
    ...
    # =========================================================
    # 4. CUSTOM MODE (unchanged logic, ClickHouse only)
    # =========================================================
    elif marketplace_id == "custom":
        ...
        count, value = client.query(custom_query, parameters=params).result_rows[0]
...
def mostSellingProducts(request):
    ...
    if start_date and end_date:
        ...
        match_conditions["order_data.order_date"] = {
            "$gte": start_date,
            "$lte": end_date,
        }
    pipeline.extend(
    ...
    if marketPlaceId and marketPlaceId != "all" and marketPlaceId != "custom":
        match = {"$match": {"product_ins.marketplace_id": ObjectId(marketPlaceId)}}
        pipeline.append(match)
    pipeline2 = [
        {
            "$group": {
    ...
    custom_match_conditions = {}
    if start_date and end_date:
        custom_match_conditions["purchase_order_date"] = {
            "$gte": start_date,
            "$lte": end_date,
        }
    custom_pipeline = [
    ...
    for i in custom_top_products:
        pipeline = [
            {"$match": {"_id": ObjectId(i["product_id"])}},
            {"$project": {"_id": None, "image_url": {"$ifNull": ["$image_url", ""]}}},
        ]
        product_image_obj = list(Product.objects.aggregate(*(pipeline)))
        try:
            i["product_image"] = product_image_obj[0].get("image_url")
    ...
    if marketPlaceId == "all":
        ...
        top_products = sorted(
            top_products, key=lambda x: x["sales_count"], reverse=True
        )[:7]
    ...
    match_pipeline = [
        {
            "$facet": {
                "current_range": [
                    {
                        "$match": {
                            "order_date": {"$gte": current_start, "$lt": now},
                            **(
                                {"marketplace_id": ObjectId(marketplace_id)}
                                if marketplace_id
                                and marketplace_id != "custom"
                                and marketplace_id != "all"
                                else {}
                            ),
                        }
                    },
                ...
                "previous_range": [
                    {
                        "$match": {
                            "order_date": {"$gte": previous_start, "$lt": previous_end},
                            **(
                                {"marketplace_id": ObjectId(marketplace_id)}
                                if marketplace_id
                                and marketplace_id != "custom"
    ...
    custom_match_pipeline = [
        {
            "$facet": {
                "current_range": [
                    {
                        "$match": {
                            "purchase_order_date": {"$gte": current_start, "$lt": now}
                        }
                    },
                    {"$group": {"_id": None, "sales_value": {"$sum": "$total_price"}}},
                ],
                "previous_range": [
                    {
                        "$match": {
        ...
        if custom_trend_data:
            for key in ["current_range", "previous_range"]:
                for item in custom_trend_data[0][key]:
                    trend_data[0][key].append(
                        {"_id": "custom", "sales_value": item["sales_value"]}
                    )
    if trend_data:
        current_range_data = {
            item["_id"]: sanitize_data(item["sales_value"])
            for item in trend_data[0]["current_range"]
        }
        previous_range_data = {
            item["_id"]: sanitize_data(item["sales_value"])
            for item in trend_data[0]["previous_range"]
        }
        if marketplace_id == "all":
            ...
            current_percentage = (
                (current_total / previous_total * 100)
                if previous_total != 0
                else (100 if current_total > 0 else 0)
            )
            ...
            trend_percentage = []
            marketplace_name = DatabaseModel.get_document(
                Marketplace.objects, {"id": marketplace_id}, ["name"]
            ).name
...
def fetchSalesSummary(request):
    ...
    if marketplace_id == "all" or marketplace_id != "custom":
        ...
        pipeline.extend(
            [
                {"$unwind": "$order_items"},
                {"$group": {"_id": None, "ids": {"$addToSet": "$order_items"}}},
                {"$project": {"_id": 0, "ids": 1}},
            ]
        )
        ...
        if summary1:
            s_pipeline = [
                {"$match": {"_id": {"$in": summary1[0]["ids"]}}},
                {
                    "$group": {
                        "_id": None,
                        "total_units_sold": {"$sum": "$ProductDetails.QuantityOrdered"},
                        "unique_product_ids": {
                            "$addToSet": "$ProductDetails.product_id"
                        },
                    }
                },
                {
            ...
            if summary2:
                ...
                data["total_sold_product_count"] = summary2[0].get(
    ...
    custom_pipeline = [
        ...
        {"$unwind": "$ordered_products"},
        {
            "$group": {
                ...
                "unique_custom_product_ids": {
                    "$addToSet": "$ordered_products.product_id"
                },
            }
        },
        {
            "$project": {
                ...
                "total_custom_sold_product_count": {
                    "$size": "$unique_custom_product_ids"
                },
            }
        },
    ]
    ...
    if custom_summary:
        ...
        total_custom_sold_product_count = custom_summary[0].get(
    ...
    if marketplace_id == "custom":
        ...
        for category in top_categories + custom_top_categories:
            ...
            if category_id in combined_categories:
                combined_categories[category_id]["total_order_value"] += category[
                    "total_order_value"
                ]
            else:
                combined_categories[category_id] = category
        sorted_categories = sorted(
            combined_categories.values(),
...
def listUsers(request):
    ...
    pipeline = [
        ...
        {"$unwind": {"path": "$role_ins", "preserveNullAndEmptyArrays": True}},
        {
            "$project": {
                "_id": 0,
                "id": {"$toString": "$_id"},
...
def exportOrderReport(request):
    ...
    for i in range(1, max_products + 1):
        product_headers.extend(
            [
                f"Product {i} Name",
                f"Product {i} SKU",
                f"Product {i} Quantity Ordered",
                f"Product {i} Quantity Shipped",
                f"Product {i} Unit Price",
            ]
        )
    ...
    for order in orders:
        ...
        if order.shipping_information:
            ...
            shipping_details = (
                f"Name: {postal_address.get('name', '')}, "
        ...
        for product_index, product in enumerate(order.order_items[:max_products]):
            product_details = [
                ...
                product.ProductDetails.QuantityOrdered,
                product.ProductDetails.QuantityShipped,
                product.Pricing["ItemPrice"]["Amount"] if product.Pricing else None,
            ]
            for col_index, value in enumerate(
                product_details, start=col + product_index * 5
            ):
                sheet.cell(row=row, column=col_index, value=value)
...
def fetchSalesSummary(request):
    ...
    if marketplace_id == "all" or marketplace_id != "custom":
        ...
        if summary:
            ...
            data["margin"] = (
                ((data["total_sales"] - data["total_cogs"]) / data["total_sales"]) * 100
                if data["total_sales"]
                else 0
            )
        else:
    ...
    if marketplace_id == "custom":
        ...
        data["total_sold_product_count"] = total_custom_sold_product_count
        data["margin"] = (
            ((total_custom_sales - total_custom_cogs) / total_custom_sales) * 100
            if total_custom_sales
            else 0
        )
    if marketplace_id == "":
        data["total_sales"] += total_custom_sales
...
def getProductVariant(request):
    ...
    product = DatabaseModel.get_document(
        Product.objects, {"id": product_id}, ["parent_sku"]
    )
    ...
    if timezone_str != "UTC":
        today_start_date, today_end_date = convertLocalTimeToUTC(
            today_start_date, today_end_date, timezone_str
        )
        start_date, end_date = convertLocalTimeToUTC(start_date, end_date, timezone_str)
    ...
    sales_data = batch_get_sales_data_optimized(
        product_ids, start_date, end_date, today_start_date, today_end_date
    )
    ...
    for order in orders:
        try:
            ...
            if not fc or not order_date:
                logger.warning(
                    f"Skipping order {order.id}: Missing fulfillment_channel or order_date."
                )
                skipped_count += 1
                continue
            if fc == "MFN":
                if not merchant_id:
                    logger.warning(
                        f"Skipping MFN order {order.id}: Missing merchant_order_id."
                    )
                    skipped_count += 1
                    continue
                ...
                if not po_id or not email:
                    logger.warning(
                        f"Skipping SellerFulfilled order {order.id}: Missing purchase_order_id or email."
                    )
                    skipped_count += 1
                    continue
    ...
    try:
        ...
        for pattern in patterns:
            try:
                ...
                continue
        logger.info(
            f"Total unique keys found for '{func_name}': {len(all_keys_to_delete)}"
        )
        deleted_count = 0
        failed_count = 0
...
def clear_cache_key(request):
    try:
        key = request.GET.get("key") or (
            request.POST.get("key") if request.method == "POST" else None
        )
        clear_all = (
            request.GET.get("all")
            or (request.POST.get("all") if request.method == "POST" else None)
        ) == "true"
        prefix = (
            request.GET.get("prefix")
            or (request.POST.get("prefix") if request.method == "POST" else None)
            or "cache"
        )
        if clear_all:
            try:
